...
namespace javacompiler {
    ...
    void JavaSemantics::add_symbol(std::string name, symbol_entry symbol) {
        ...
        bool search_global = (this->symbol_table.size() == 1)?true:false; 
    ...
    void JavaSemantics::add_arg_to_current(std::string name, std::string type) {
        ...
        scope current_scope = this->symbol_table[this->symbol_table.size()-1];
        ...
        this->symbol_table[this->symbol_table.size()-1].symbols.at(current_method).args.push_front({name, type}); // since we're ascending , the insertion needs to happen in reverse
    }
    ...
    void JavaSemantics::set_current_method_call(std::string name)  {
        ...
        if (this->symbol_table[0].symbols.count(name) == 0 ) {
            this->throw_error("no method with name "+ highlight(name) + " declared");
        }

        symbol_entry symbol = this->symbol_table[0].symbols.at(name);
    ...
    void JavaSemantics::check_args_number() {
        ...
        int expected = this->symbol_table[0].symbols.at(current_method_call).args.size();
